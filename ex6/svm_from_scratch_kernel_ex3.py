""" Building an SVM from scratch, including kernel, evaluated on dataset 3 """
...
def gradient_descent(theta, inputs, correct, C, learning_rate=0.000001, iterations=10000, decay_term=1, old=False):
...
def show_decision_boundary(theta):
    ...
    X1_increments = np.linspace(-0.7, 0.4, 100)
    X2_increments = np.linspace(-0.75, 0.6, 100)
...
# optimize theta
final_theta = gradient_descent(initial_theta, gaussian_features, y, C=300)
